...
import KerpFullScreenModal from './Modal';
...
void main() {
    ...
    // Apply a subtle KOC-style blue tint while keeping image detail
    vec3 brandBlue = vec3(0.0, 0.45, 0.75);
    outColor.rgb = mix(outColor.rgb, brandBlue, 0.22);
    outColor.a *= vAlpha;
}
`;
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: MenuItem[], onActiveItemChange: ActiveItemCallback, onMovementChange: MovementChangeCallback, onInit?: InitCallback) {
    this.canvas = canvas;
    this.items = items;
  ...
  private render(): void {
    ...
    // Clear to a light KOC-aligned blue so the canvas background is not black
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const handleOpenLink = () => {
    ...
    if (activeItem.link.startsWith('http')) {
      ...
      // For SPA routing, replace this with your router navigation.
  ...
  return (
    <div className="relative h-full w-full bg-white">
      ...
      {/* Mobile: title/description over the image in white for contrast */}
      {activeItem && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center px-6 text-center md:hidden">
      ...
      {/* Desktop / large: caption below the disc, as before */}
      {activeItem && (
        <div className="pointer-events-none absolute inset-x-0 bottom-8 hidden flex-col items-center gap-1 px-4 text-center md:flex">
          <h2 className="text-lg font-semibold text-sky-700 md:text-xl">
            {activeItem.title}
          </h2>
          <p className="text-md text-gray-700 md:text-sm max-w-[22ch]">
            {activeItem.description}
          </p>
        </div>
      )}
      ...
      {activeItem && (
        <>
          {/* Large overlaid title/description were removed to avoid overlap on small screens.
              We now show title/description in a separate caption block below the disc. */}
          ...
          ${
            isMoving
              ? 'top-[64%] md:top-[68%] opacity-0 pointer-events-none duration-[100ms] scale-0 -translate-x-1/2'
              : 'top-[70%] md:top-[72%] opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
          }
        `}
          >
      ...
      <AnimatePresence>
        {isModalOpen && activeItem && (
          <KerpFullScreenModal
            key={activeItem.title || 'kerp-modal'}
            item={activeItem}
